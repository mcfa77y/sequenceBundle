	...
	function renderProgress(data) {
		...
		if (data.errorMessage && data.errorMessage.length > 0) {
			if (data.errorMessage.contains("1000")) {
				var errorMessage = "FASTA format is valid, but your data is too large (it has "
					+ data.sequenceCount
					+ " sequences, each "
...
var filename = Utils.getFilename(wp);
	...
	$('.upload-minimize-button').each(function() {
		$(this).click(function() {
			...
			var skipId = $(this).attr('data-target');
			...
			for (var i=0; i<openPanels.length; i++){
				...
				if(skipId !== panel.attr('id')){
					...
					// swap + for -
					$(panel.siblings()[0].children[1].children[0].children[0]).removeClass('glyphicon-minus').addClass('glyphicon-plus');
				}
			}
		});
	});
